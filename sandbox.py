...
# Função que converte uma data no formato ISO (YYYY-MM-DD) para um objeto datetime.date, 
# que é o tipo correto para trabalhar com datas no Python e no st.data_editor.
def to_date(valor):
    # Se o valor for None (ex: campo vazio no banco)
    # não há nada para converter
    if valor is None:
        return None

    # Converte string no formato ISO (YYYY-MM-DD)
...
with plano_trabalho:
    ...
    # -------------------------
        ...
        if opcao_editar_pt == "Atividades":
            ...
            for a in atividades_exist:
                lista_atividades.append({
                    ...
                    "data_inicio": parse_date_br(a.get("data_inicio")),
                    "data_fim": parse_date_br(a.get("data_fim")),
                })
            ...
            # Converter datas para string (necessário para TextColumn)
            df_atividades["data_inicio"] = df_atividades["data_inicio"].apply(
                lambda x: x.strftime("%d/%m/%Y") if pd.notna(x) else ""
            )

            df_atividades["data_fim"] = df_atividades["data_fim"].apply(
                lambda x: x.strftime("%d/%m/%Y") if pd.notna(x) else ""
            )
            ...
            if salvar_ativ:
                ...
                for i, row in df_editado.iterrows():
                    ...
                    data_inicio = parse_date_br(row["data_inicio"])
                    data_fim = parse_date_br(row["data_fim"])

                    # Validações
                    if not atividade:
                        erros.append(f"Linha {i + 1}: atividade obrigatória.")

                    if not data_inicio:
                        erros.append(f"Linha {i + 1}: data de início inválida.")

                    if not data_fim:
                        erros.append(f"Linha {i + 1}: data de fim inválida.")

                    if data_inicio and data_fim and data_inicio > data_fim:
                        erros.append(f"Linha {i + 1}: data inicial maior que a data final.")
        ...
        #         lista_atividades.append({
        #             "atividade": a.get("atividade", ""),
        #             "data_inicio": to_date(a.get("data_inicio")),
        #             "data_fim": to_date(a.get("data_fim")),
        #         })
        ...
        #                 erros.append(f"Linha {i+1}: datas obrigatórias.")

        #             if data_inicio and data_fim and data_inicio > data_fim:
        #                 erros.append(f"Linha {i+1}: data inicial maior que final.")
